...
// "I've been so busy making sure everyone here has food that
// I completely forgot to check why we stopped getting more sand!
...
function parseMap(input: string): Mapping[] {
...
function mapNumber(originalNumber: number, maps: Mapping[]): number {
  const correctMapping = maps.find((m) => {
    return originalNumber >= m.source && originalNumber < m.source + m.range;
  });

  if (!correctMapping) return originalNumber;
  return originalNumber - correctMapping.source + correctMapping.destination;
}

export default function solve(input: string): [number, number] {
  const lines = input.split("\n\n");
  const seeds = lines.shift()!.split("seeds: ")[1].split(" ").map((n) =>
    parseInt(n, 10)
  );

  console.log(seeds);

  const maps = lines.map((l) => parseMap(l.split(":\n")[1]));

  const part_1 = Math.min(...seeds.map((seed) => {
    maps.forEach((map) => {
      seed = mapNumber(seed, map);
    });

    return seed;
  }));

  let smallestSeed = Number.MAX_VALUE;

  for (let i = 0; i + 1 < seeds.length; i = i + 2) {
    const start = seeds[i];
    ...
    for (let j = 0; j < range; j++) {
      let num = start + j;
      maps.forEach((map) => {
        num = mapNumber(num, map);
      });

      smallestSeed = Math.min(smallestSeed, num);
    }

    console.log(smallestSeed);
  }

  return [part_1, smallestSeed];
}
