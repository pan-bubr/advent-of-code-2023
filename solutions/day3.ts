...
export default function solve(input: string): [number, number] {
  const numbers: FoundNumber[] = findNumbers(input);

  const part_1 = numbers.filter((n: FoundNumber) => isValidNumber(input, n))
    .map(
      (n) => n.n,
    ).reduce((acc, n) => acc + n, 0);

  const part_2 = findGears(input).filter((g) => g.number2).map((g) => {
    return g.number1.n * g.number2.n;
  }).reduce((acc, n) => acc + n, 0);
...
export function findNumbers(input: string): FoundNumber[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    for (let j = 0; j < lines[0].length; j++) {
      ...
      if (!Number.isNaN(parseInt(symbol))) {
        let new_number = symbol;
        j++;
        while (!Number.isNaN(parseInt(lines[i][j]))) {
          new_number += lines[i][j];
          j++;
        }
        foundNumbers.push({
          n: parseInt(new_number),
          x,
          y,
        });
      }
...
export function isValidNumber(
  ...
  const lines = input.split("\n");
  const numberLength = foundNumber.n.toString().length;

  const symbols = "*/=@&$+-%#";

  for (let i = foundNumber.x - 1; i <= foundNumber.x + 1; i++) {
    for (let j = foundNumber.y - 1; j <= foundNumber.y + numberLength; j++) {
...
export function findGears(input: string): FoundGear[] {
  const foundGears: FoundGear[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    for (let j = 0; j < lines[0].length; j++) {
      const symbol = lines[i][j];
      if (symbol == "*") {
        const [number1, number2] = findSurroundingNumbers(input, i, j);
...
function findSurroundingNumbers(input: string, x, y): FoundNumber[] {
  const lines = input.split("\n");
  ...
  for (let i = x - 1; i <= x + 1; i++) {
    for (let j = y - 1; j <= y + 1; j++) {
      ...
      if (!Number.isNaN(parseInt(lines[i][j]))) {
        while (j > 0 && !Number.isNaN(parseInt(lines[i][j - 1]))) {
          j--;
        }
